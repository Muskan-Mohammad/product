...
function CardList({mode}) {
  ...
  return (
    ...
    {filteredData.map((item) => (
      ...
      <div key={item.id} className="col" style={{margin:'70px 10px'}}>
      ...
      <div className="card h-100" style={{ backgroundColor: '#FDCEDF'  }}>
        ...
        <video src={item.vedio} autoPlay loop style={{ height: '250px', width: '350px', position: 'relative', top: '-10px', left: '50px' }} />
        <div style={{ display: 'flex' }}>
  {[1, 2].map((index) => (
    <img
      key={index}
      src={item.image}
      style={{ height: '250px', width: '200px', margin: '20px 20px' }}
      alt="..."
    />
  ))}
</div>
