...
function SeperateCard() {
  ...
  return (
    ...
    <div style={{width:'450px' , margin:'10px 10px' , height:'100%'}}>
    <div className="card h-100">
    <b style={{marginLeft:'50px'}}>{item.hashtag}</b>
    <video src={item.vedio} autoPlay muted  loop style={{height:'400px' , width:'430px' , position:'relative', top:'-30px', left:'10px'}}/>
    <div style={{ display: 'flex' }}>
  {[1, 2].map((index) => (
    <img
      key={index}
      src={item.image}
      style={{ height: '250px', width: '200px', margin: '-60px 10px' }}
      alt="..."
    />
  ))}
</div>
